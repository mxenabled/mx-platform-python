...
> InsightsResponseBody list_insights_by_transaction(transaction_guid, user_guid, page=page, records_per_page=records_per_page)
...
with mx_platform_python.ApiClient(configuration) as api_client:
    ...
    try:
        # List insights by transaction
        api_response = api_instance.list_insights_by_transaction(transaction_guid, user_guid, page=page, records_per_page=records_per_page)
        print("The response of InsightsApi->list_insights_by_transaction:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling InsightsApi->list_insights_by_transaction: %s\n" % e)
```
...
with mx_platform_python.ApiClient(configuration) as api_client:
    ...
    try:
        # Read insight
        api_response = api_instance.read_insight_user(user_guid, insight_guid)
        print("The response of InsightsApi->read_insight_user:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling InsightsApi->read_insight_user: %s\n" % e)
```
...
with mx_platform_python.ApiClient(configuration) as api_client:
    ...
    user_guid = 'USR-fa7537f3-48aa-a683-a02a-b18940482f54' # str | The unique identifier for a `user`, beginning with the prefix `USR-`.
    insight_guid = 'BET-1234-abcd' # str | The unique identifier for the insight. Defined by MX.
    insight_update_request_body = mx_platform_python.InsightUpdateRequestBody() # InsightUpdateRequestBody | The insight to be updated (None of these parameters are required, but the user object cannot be empty.)

    try:
        # Update insight
        api_response = api_instance.update_insight(user_guid, insight_guid, insight_update_request_body)
